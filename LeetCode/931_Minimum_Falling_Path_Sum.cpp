...
public:
    int recursion(int i, int j, vector<vector<int>> &matrix)
    {
        ...
        return min(leftDiagonal, min(down, rightDiagonal));
    }
    ...
    int memoization(int i, int j, vector<vector<int>> &matrix, vector<vector<int>> &dp)
    {
        ...
        return dp[i][j] = min(leftDiagonal, min(down, rightDiagonal));
    }
    ...
    int tabulation(int m, int n, vector<vector<int>> &matrix)
    {
        ...
        for (int i = m - 2; i >= 0; i--)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                dp[i][j] = min(leftDiagonal, min(down, rightDiagonal));
            }
        }
    ...
    int spaceOptimization(int m, int n, vector<vector<int>> &matrix)
    {
        ...
        for (int i = m - 2; i >= 0; i--)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                cur[j] = min(leftDiagonal, min(down, rightDiagonal));
            }
            prev = cur;
        }
